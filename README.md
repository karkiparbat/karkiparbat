...
# GitHub username
USER = os.getenv("GITHUB_USER", "karkiparbat")

# Output folder
OUT_DIR = "output"
OUT_FILE = f"{OUT_DIR}/github-contribution-grid-snake.svg"
os.makedirs(OUT_DIR, exist_ok=True)

# Get contribution SVG from GitHub
...
# Create SVG
svg = E('svg', xmlns='http://www.w3.org/2000/svg', version='1.1',
        width=str(SVG_W), height=str(SVG_H), viewBox=f"0 0 {SVG_W} {SVG_H}")

SubElement(svg, 'rect', x='0', y='0', width=str(SVG_W), height=str(SVG_H), fill='white')

# Create grid and snake path
...
for ci, week in enumerate(weeks_sorted):
    for ri in range(ROWS):
        ...
        color = COLORS[val]
        SubElement(grid, 'rect', x=str(x), y=str(y), width=str(CELL), height=str(CELL),
...
SubElement(snake, 'animateTransform', attributeName='transform', attributeType='XML',
           type='scale', values='1;1.1;1', dur='1.5s', repeatCount='indefinite')
